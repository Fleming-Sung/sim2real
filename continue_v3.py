...
def get_state(o, g):
    ...
    # state[0:8] = (state[0:8] - 4.5 )/4.5
    # state[8] = state[8] / np.pi
    return state
...
if __name__ == "__main__":
    ...
    for i in range(1000000):
        ...
        if i < max_explore:
            ...
            # 卡顿 就 设置一个临时目标
            if flag_stuck :
                # noise = np.random.normal(0,1,np.shape(state)) * 0.1
                # state_nd = state + noise
                # state_nd[0:8] = state_nd[0:8].clip(0, 9)
                # state_nd[8] = state_nd[8].clip(-np.pi, np.pi)
                laser_data = obs["laser"]
                max_index = np.argmax(laser_data)
                angle = angle_list[max_index]
                state_nd = [laser_data[0], laser_data[10], laser_data[20], laser_data[30], laser_data[40],
                            laser_data[50], laser_data[60], 0.5, angle]
                action = policy.select_action(state_nd)
        ...
        queue[len_stuck-1] = dist_next

        # 之前第20步 与 当前步的位姿差

        if(abs(queue[0] - queue[len_stuck-1]) <= 0.1):
            flag_stuck = True
